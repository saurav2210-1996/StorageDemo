...
abstract class KnoxLicenseReceiver : BroadcastReceiver() {

    private val DEFAULT_ERROR_CODE = -1
    companion object{
        val ACTION_ELM_LICENSE_STATUS = "edm.intent.action.license.status"
        val ACTION_KLM_LICENSE_STATUS = "edm.intent.action.knox_license.status"
        val EXTRA_KLM_ERROR_CODE = "edm.intent.extra.knox_license.errorcode"
        val EXTRA_ELM_ERROR_CODE = "edm.intent.extra.license.errorcode"
    }
        ...
        if (intent == null) {
            ...
            if (action == KnoxEnterpriseLicenseManager.ACTION_LICENSE_STATUS) {
                ...
                if (errorCode == KnoxEnterpriseLicenseManager.ERROR_NONE) {
                    ...
                    onLicenseActivationFailed(context, errorMessage, errorCode)
    ...
    abstract fun onKnoxLicenseActivated(context: Context?)

    abstract fun onBackwardLicenseActivated(context: Context?)

    abstract fun onLicenseActivationFailed(context: Context?, errorMessage: String,errorCode: Int)
}